...
def actions():
    ...
    return ActionChains(fixture.get_driver())
...
class WaitingFinder(object):
    ...
    def _do(self, command):
        ...
        :return: self
        """
        print(u'Делаем команду: %s' % command)
...
class RootSElement(object):
    ...
    def __getattr__(self, item):
        return getattr(fixture.get_driver(), item)
...
class SElement(LoadableContainer, WaitingFinder, Filler):
    ...
    def smartclick(self):
        for i in range(20):
            a = self.location  # Эта строка бессмысленна, но без нее вылетает "StaleElementReferenceException"
            if self.found.is_selected() == False:
                time.sleep(0.2)
            else:break
        return self._do(lambda: self.found.click())
    ...
    def is_clicable(self):
        for i in range(10):
            ...
            try:
                ...
                time.sleep(0.2)
        raise AssertionError('Element not clicable')


    def is_enabled(self):
        return self._execute(lambda: self.found.is_enabled())

    def is_custom_disabled(self):
        if 'disable' in self.found.get_attribute('class'):
            return True
        else:
            return False

    def is_custom_enabled(self):
        if 'disable' not in self.found.get_attribute('class'):
            return True
        else:
            return False
